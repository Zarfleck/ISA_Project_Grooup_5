...
/**
 * THIS IS NOT EXPRESS SERVER. NAMED IT JUST FOR GIT REPLACEMENT IN DEPLOYMENT WITH EXPRESSSERVER.JS
 */
...
try {
  ...
  console.log(
    "Dotenv not available, using platform environment variables only"
  );
}
...
function parsePostData(request) {
  // Declares function that takes HTTP request object as parameter
  return new Promise((resolve, reject) => {
    // Returns Promise for async operation with resolve/reject callbacks
    let body = ""; // Initialize empty string to accumulate all data chunks
    request.on("data", (chunk) => {
      // Listen for 'data' events when chunks of data arrive from client
      body += chunk.toString(); // Convert Buffer chunk to string and append to body variable
    });
    request.on("end", () => {
      // Listen for 'end' event when all data has been received
      try {
        // Start try-catch block to handle potential JSON parsing errors
        ...
        // If JSON parsing fails, catch the error
        reject(error); // Reject Promise with the parsing error
      }
    });
    request.on("error", (error) => {
      // Listen for any errors during request processing
...
async function handleInsertDefault(response, userId) {
  // Async function to handle inserting default patients, takes HTTP response object
  try {
    // Start try-catch block to handle any database errors
...
// This block of code below was assisted by Claude Sonnet 4 (https://claude.ai/)
async function handleCustomQuery(request, response, userId) {
  // Async function to handle custom SQL queries from client
  try {
    // Start try-catch block to handle parsing and database errors
    const data = await parsePostData(request); // Parse the JSON data from POST request body

    if (!data.query) {
      // Check if the query field is missing or empty
    ...
    // If any error occurs during request parsing or database operation
    response.writeHead(500); // Set HTTP status code to 500 (Internal Server Error)
    response.end(
      JSON.stringify({
        // Send error response as JSON
...
// This block of code below was assisted by Claude Sonnet 4 (https://claude.ai/)
async function handleGetQuery(request, response, userId) {
  // Async function to handle GET requests with SQL queries in URL path
  try {
    // Start try-catch block to handle URL parsing and database errors
    ...
    if (pathParts.length >= 4 && pathParts[3] === "sql") {
      // Check if URL has at least 4 parts and 4th part is 'sql' (/api/v1/sql/query)
      const query = decodeURIComponent(pathParts[4] || ""); // Extract and decode the SQL query from 5th path segment (URL encoded)

      if (!query) {
        // Check if query is empty or missing after decoding
      ...
      // If URL path doesn't match expected pattern (/api/v1/sql/...)
    ...
    // If any error occurs during URL parsing or database operation
...
const server = http.createServer(async (request, response) => {
  // Create HTTP server with async callback for each request
  ...
  try {
    // Start try-catch block to handle any routing or processing errors
    ...
    if (request.method === "POST") {
      // Check if incoming request is a POST method
      if (pathname === "/api/v1/patients/default") {
        // Route for inserting default patients via POST
        await handleInsertDefault(response, userId); // Call function to insert 4 predefined patients
      } else if (pathname === "/api/v1/sql") {
        // Route for custom SQL queries via POST
        await handleCustomQuery(request, response, userId); // Call function to handle custom SQL from request body
      } else {
        // If POST request doesn't match any known endpoints
      ...
      if (pathname.startsWith("/api/v1/sql/")) {
        // Route for SQL queries embedded in URL path (for SELECT)
        await handleGetQuery(request, response, userId); // Call function to handle SQL query from URL
      } else {
        // If GET request doesn't match any known endpoints
        response.writeHead(404); // Set HTTP status to 404 (Not Found)
        response.end(
          JSON.stringify({
            // Send error response as JSON
      ...
      // If request method is neither POST nor GET (e.g., PUT, DELETE, PATCH)
    ...
    // If any unexpected error occurs during request processing
    response.writeHead(500); // Set HTTP status to 500 (Internal Server Error)
    response.end(
      JSON.stringify({
        // Send error response as JSON
...
async function startServer() {
  // Async function to initialize and start the HTTP server
  ...
  server.listen(port, host, () => {
    // Start the HTTP server listening on specified port and host
    console.log(`${STRINGS.SERVER.STARTUP} ${port} on ${host}`); // Log server startup message with port and host
  });
}

process.on("SIGINT", async () => {
  // Listen for Ctrl+C (SIGINT signal) to gracefully shutdown
